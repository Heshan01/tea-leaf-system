...
export default function MessagesTab() {
  ...
  // realtime messages (broadcast + vehicle)
  useEffect(() => {
    if (!vehicleId) return;
  ...
  const markAsRead = async (msgId: string) => {
    try {
      await updateDoc(doc(db, "messages", msgId), { readBy: arrayUnion(uid) });
    } catch {
      // no popup
    }
  };
  ...
  const Chip = ({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) => (
    ...
    >
      <Text style={{ color: active ? THEME.green : THEME.muted, fontWeight: "900" }}>{label}</Text>
    </Pressable>
  );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: THEME.bg }}>
      ...
      <View style={{ padding: P, paddingBottom: 10, gap: 12 }}>
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            {/* Back → Home */}
            <Pressable onPress={() => router.replace("/(tabs)/home")} style={{ padding: 6 }}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={{ padding: P, paddingTop: 6, gap: 12, paddingBottom: 30 }}
          ...
          ListEmptyComponent={
            ...
            >
              ...
              <Text style={{ color: THEME.muted }}>Admin broadcast හෝ vehicle message එවලා තිබුනොත් මෙතන පේනවා.</Text>
            </View>
          }
          renderItem={({ item }) => {
